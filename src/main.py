...
class PerformanceFunctionAnalyzer:
    ...
    def _fit_model(self, model_type: str, train_df: pd.DataFrame, test_df: Optional[pd.DataFrame], lang: str, pivot_size: int) -> Tuple[Any, pd.DataFrame]:
        ...
        if test_df is not None:
            if model_type == "amue":
                ...
                return fit_nd_eval_amue_model_diff_test(train_df, test_df, lang, pivot_size)
            elif model_type == "gpr":
                from src.fit_performance_functions import fit_nd_eval_gpr_model_diff_test
                return fit_nd_eval_gpr_model_diff_test(train_df, test_df, lang, pivot_size)
        else:
            if model_type == "amue":
                from src.fit_performance_functions import fit_nd_eval_amue_model
                return fit_nd_eval_amue_model(train_df, lang, pivot_size)
            elif model_type == "gpr":
                from src.fit_performance_functions import fit_nd_eval_gpr_model
                return fit_nd_eval_gpr_model(train_df, lang, pivot_size)
    ...
    def run_fit_and_eval(self) -> None:
        ...
        logger.info("--- Starting AMUE Model Evaluation ---")
        ...
        pred_file = os.path.join(pred_dir, f"amue_pred_nd_errors_lang_{self.config.lang}_pivot_{self.config.pivot_size}.csv")
        ...
        logger.info("--- Starting GPR Model Evaluation ---")
    ...
    def analyze_expansion_paths(self) -> None:
        ...
        for (lang, pivot_size), max_y in max_f1_scores.items():
            ...
            a0 = params[0]
            ys = np.linspace(np.ceil(a0), max_y, int((max_y - a0) // 1) + 1)
